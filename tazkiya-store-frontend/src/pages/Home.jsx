    ...
    <Fragment>
      <div className="flex gap-1 backdrop-blur-md rounded p-2 rounded">
        <input
          ...
          className="w-[70%] p-2 clr-text-dark border-2 border-sky-300 rounded focus:outline-sky-400"
          type="text"
        />
        <div className="flex border rounded">
          <img
            ...
            className={`w-10 p-2 ${sortProductsCol ? "bg-sky-200" : ""}`}
            src="/SVGs/bars.svg"
          />
          ...
          >
            <img className="w-10" src="/SVGs/sort.svg" alt="sort" />
          </div>
      ...
      <div
        className={`p-5 grid ${sortProductsCol ? "grid-cols-1" : "grid-cols-2"} md:grid-cols-3 gap-5`}
      >
        {products.length > 0 ? (
          categorizedProducts.slice(0, visibleProducts).map((p) => {
            return (
              ...
              >
                ...
                <div
                  className={`bg-sky-200 rounded absolute bottom-0 left-0 p-2 border-2 border-sky-100 ${sortProductsCol ? "h-[100%] w-[150px]" : "h-[110px] w-full"}`}
                >
                  <h2 className="font-bold text-sm clr-text-dark whitespace-nowrap overflow-x-auto text-ellipsis">
                    {p.name}
                  </h2>
                  <p className="text-gray-600 text-[10px] clr-text-darkSub whitespace-nowrap overflow-x-auto">
                    {p.description}
                  </p>
          ...
          <div className="w-full flex justify-center">
            <img className="w-10" src="/SVGs/spin.svg" alt="Loading..." />
          </div>
        )}
      </div>
      {visibleProducts < products.length && (
        <div onClick={loadMoreProducts} className="flex justify-center mb-3">
          <button className="border clr-text-light bg-sky-400 px-3 p-2 rounded shadow">
            More Products
          </button>
        </div>
      )}
